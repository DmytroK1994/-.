(function () {
  ...
  const CATALOG = {
    ...
    "Мова": ["Language", "言語", "语言", "Langue", "Sprache", "Язык"],
    "Теми": ["Themes", "テーマ", "主题", "Thèmes", "Farbschemata", "Темы"],
    "Резервна копія": ["Backup", "バックアップ", "备份", "Sauvegarde", "Sicherung", "Резервная копия"],
    "Інші налаштування": ["Other settings", "その他の設定", "其他设置", "Autres paramètres", "Weitere Einstellungen", "Другие настройки"],
    "Обери потрібну категорію.": ["Choose a category.", "カテゴリーを選択してください。", "请选择一个类别。", "Choisissez une catégorie.", "Kategorie auswählen.", "Выберите нужную категорию."],
    "Нижче є ще пункти ↓": ["More items below ↓", "下にさらに項目があります ↓", "下方还有更多项目 ↓", "D’autres éléments se trouvent plus bas ↓", "Unten gibt es weitere Einträge ↓", "Ниже есть ещё пункты ↓"],
    "↑ Вище є ще пункти": ["↑ More items above", "↑ 上にさらに項目があります", "↑ 上方还有更多项目", "↑ D’autres éléments se trouvent plus haut", "↑ Oben gibt es weitere Einträge", "↑ Выше есть ещё пункты"],
    "↑ Є інші пункти ↓": ["↑ More items above and below ↓", "↑ 上下にさらに項目があります ↓", "↑ 上下方还有更多项目 ↓", "↑ D’autres éléments se trouvent plus haut et plus bas ↓", "↑ Oben und unten gibt es weitere Einträge ↓", "↑ Выше и ниже есть ещё пункты ↓"],
    "Категорії налаштувань розділено на окремі сторінки, а список показує чотири пункти та індикатор продовження.": ["Settings categories are now separate pages, and the list shows four items with a continuation indicator.", "設定カテゴリーを個別のページに分け、一覧には4項目と続きのインジケーターを表示するようにしました。", "设置类别现已分为独立页面，列表可显示四个项目并带有更多内容提示。", "Les catégories de paramètres sont désormais séparées en pages distinctes, et la liste affiche quatre éléments avec un indicateur de continuation.", "Die Einstellungskategorien sind jetzt auf separate Seiten verteilt; die Liste zeigt vier Einträge und einen Fortsetzungsindikator.", "Категории настроек разделены на отдельные страницы, а список показывает четыре пункта и индикатор продолжения."],
    ...
    "Курсор у всіх полях введення тепер автоматично встановлюється після останнього символу.": ["The cursor now automatically moves after the last character in every input field.", "すべての入力欄でカーソルが自動的に末尾へ移動するようになりました。", "所有输入框中的光标现在都会自动移到最后一个字符之后。", "Le curseur se place désormais automatiquement après le dernier caractère dans tous les champs de saisie.", "Der Cursor wird nun in allen Eingabefeldern automatisch hinter das letzte Zeichen gesetzt.", "Курсор во всех полях ввода теперь автоматически устанавливается после последнего символа."],
    "Прибрано громіздку панель математичних операторів. Поле окремої ваги знову стало компактним.": ["The bulky math-operator panel has been removed. The separate-weight field is compact again.", "大きな演算パネルを削除し、個別重量の入力欄を再びコンパクトにしました。", "已移除占用空间的数学运算面板，单独重量输入框恢复紧凑。", "Le panneau encombrant des opérateurs mathématiques a été supprimé. Le champ du poids séparé est de nouveau compact.", "Die sperrige Leiste mit Rechenoperatoren wurde entfernt. Das Feld für das Einzelgewicht ist wieder kompakt.", "Громоздкая панель математических операторов удалена. Поле отдельного веса снова стало компактным."],
    "Розділ керування перетворено на впорядкований центр налаштувань з окремими категоріями.": ["The management section is now an organized settings hub with separate categories.", "管理セクションをカテゴリー別の設定センターに整理しました。", "管理部分已重组为分类清晰的设置中心。", "La section de gestion est devenue un centre de paramètres organisé par catégories.", "Der Verwaltungsbereich wurde zu einer übersichtlichen Einstellungszentrale mit einzelnen Kategorien.", "Раздел управления преобразован в упорядоченный центр настроек с отдельными категориями."],
